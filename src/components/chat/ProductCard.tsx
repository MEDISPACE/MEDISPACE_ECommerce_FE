...
export function ProductCard({ product, isOwnMessage }: ProductCardProps) {
  ...
  return (
    <div
      className={`rounded-xl overflow-hidden shadow-md w-[220px] border ${
    ...
    >
      ...
      <div className='relative w-full h-32 bg-gray-100 overflow-hidden flex items-center justify-center'>
        ...
        {product.requiresPrescription && (
          <Badge className='absolute top-2 left-2 bg-orange-500 text-white text-[10px] px-1.5 py-0.5'>Kê đơn</Badge>
        )}
      </div>
      ...
      <div className={`p-3 ${isOwnMessage ? 'text-white' : 'text-gray-900'}`}>
        ...
        <p className={`text-xs mb-1 ${isOwnMessage ? 'text-white/70' : 'text-gray-400'}`}>{product.unit}</p>
        <p className={`text-base font-bold ${isOwnMessage ? 'text-cyan-200' : 'text-blue-600'}`}>{formattedPrice}</p>
      </div>

      {/* CTA */}
      <div className='px-3 pb-3'>
        <a href={`/products/${product.slug}`} target='_blank' rel='noopener noreferrer' className='block w-full'>
          <Button
            size='sm'
