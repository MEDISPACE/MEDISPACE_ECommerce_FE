...
export function FloatingChatWidget() {
    ...
    return (
        <>
                ...
                <div className="fixed bottom-24 right-6 z-50 w-[90vw] sm:w-[360px] h-[80vh] sm:h-[550px] max-h-[600px] bg-white rounded-2xl shadow-2xl border-2 border-blue-100 flex flex-col overflow-hidden slide-up-animation">
                    ...
                    <div className="bg-gradient-to-r from-blue-600 to-cyan-500 text-white p-4 flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-1">
                            <Button variant="ghost" size="sm" onClick={handleMinimize} className="text-white hover:bg-white/20 h-8 w-8 p-0">
                    ...
                    <div className="flex-1 overflow-hidden flex flex-col">
                        {!isAuthenticated ? (
                            <div className="flex flex-col items-center justify-center h-full p-6 text-center space-y-4">
                                ...
                                <p className="text-gray-600 text-sm max-w-[240px]">Bạn cần đăng nhập để được dược sĩ tư vấn và hỗ trợ tốt nhất.</p>
                                <div className="flex flex-col gap-2 w-full max-w-[200px]">
                                    <a href="/login" className="w-full">
                                        <Button className="w-full bg-gradient-to-r from-blue-600 to-cyan-500 hover:from-blue-700 hover:to-cyan-600 text-white">Đăng nhập ngay</Button>
                                    </a>
                                    <a href="/register" className="w-full">
                                        <Button variant="outline" className="w-full border-blue-200 text-blue-600 hover:bg-blue-50">Đăng ký tài khoản</Button>
                                    </a>
                                </div>
                            </div>
                        ) : isLoading ? (
                            ...
                            <div className="flex flex-col items-center justify-center h-full p-6 text-center">
                                ...
                                <p className="text-gray-600 text-sm mb-4">Gửi tin nhắn cho dược sĩ để được tư vấn.<br />Dược sĩ sẽ trả lời khi có thời gian.</p>
                                <Button onClick={handleCreateConversation} disabled={isLoading} className="bg-gradient-to-r from-blue-600 to-cyan-500 hover:from-blue-700 hover:to-cyan-600 text-white">
                                    Bắt đầu chat
                                </Button>
                            </div>
                        )}
            ...
            {/* Minimized State */}
            {isOpen && isMinimized && (
                <div className="fixed bottom-24 right-6 z-50 bg-gradient-to-r from-blue-600 to-cyan-500 text-white rounded-lg shadow-xl px-4 py-3 cursor-pointer hover:shadow-2xl transition-shadow" onClick={() => setIsMinimized(false)}>
                    <div className="flex items-center gap-2">
                        <MessageCircle className="w-5 h-5" />
