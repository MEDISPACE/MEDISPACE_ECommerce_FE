...
export function ReturnManagementPage({ role = 'admin' }: ReturnManagementPageProps) {
    ...
    const [pendingAction, setPendingAction] = useState<{ request: ReturnRequest; action: 'approved' | 'rejected' } | null>(null)
    ...
    const reviewMutation = useMutation({
        mutationFn: (data: { requestId: string; status: 'approved' | 'rejected'; approvedAmount?: number; reviewNotes?: string; rejectionReason?: string }) =>
    ...
    const refundMutation = useMutation({
        mutationFn: (data: { requestId: string; refundedAmount: number; refundTransactionId?: string; refundNotes?: string }) =>
    ...
    const toggleSelectAll = () => {
        ...
        const selectableRequests = requests.filter(
            (r: ReturnRequest) => r.status === 'pending' || r.status === 'reviewing'
        )
                            ...
                            <tr>
                                <th className="p-4">
                                    <Checkbox
                                        checked={
                                            selectedRequests.length > 0 &&
                                            selectedRequests.length === requests.filter(
                                                (r: ReturnRequest) => r.status === 'pending' || r.status === 'reviewing'
                                            ).length
                                        }
            ...
            <Dialog open={reviewDialogOpen} onOpenChange={setReviewDialogOpen}>
                <DialogContent>
                    ...
                    <DialogFooter>
                        ...
                        <Button
                            className={reviewAction === 'approved' ? '!text-white !bg-green-600 hover:!bg-green-700' : '!text-white !bg-red-600 hover:!bg-red-700'}
                            onClick={() => {
                                if (selectedRequest) {
                ...
                <AlertDialogContent>
                    ...
                    <AlertDialogFooter>
                        ...
                        <AlertDialogAction
                            ...
                            className={pendingAction?.action === 'approved'
