...
import { AlertCircle, CalendarClock, CheckCircle, Copy, Loader2, RefreshCw, ShoppingCart, Ticket, Truck } from 'lucide-react'
...
function getStatusDetail(coupon: MyCoupon) {
  if (coupon.userCouponStatus === 'needs_min_order') return `Áp dụng cho đơn từ ${formatCurrency(coupon.minOrderAmount || 0)}`
  if (coupon.userCouponStatus === 'used_up') return `Bạn đã dùng ${coupon.userUsageCount || 0}/${coupon.userUsageLimit || 1} lượt`
  ...
  return coupon.minOrderAmount ? `Đơn từ ${formatCurrency(coupon.minOrderAmount)}` : 'Không yêu cầu giá trị đơn tối thiểu'
}
        ...
        <div className='flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between'>
          <div className='min-w-0 flex-1'>
            <div className='mb-2 flex flex-wrap items-center gap-2'>
              <code className='rounded-md bg-[#F0F6FF] px-2.5 py-1 text-sm font-bold text-[#0A2463]'>{coupon.code}</code>
              <Badge className={status.className}>{status.label}</Badge>
              {coupon.isTargeted && <Badge className='bg-[#E8EDF5] text-[#0A2463] hover:bg-[#E8EDF5]'>Dành riêng cho bạn</Badge>}
            </div>
            ...
            <div className='mt-3 grid gap-2 text-sm text-gray-600 sm:grid-cols-2'>
              <div className='flex items-center gap-2'>
                {coupon.type === 'free_shipping' ? <Truck className='h-4 w-4 text-[#1E40AF]' /> : <Ticket className='h-4 w-4 text-[#1E40AF]' />}
            ...
            {coupon.excludePrescriptionItems && <p className='mt-1 text-xs text-gray-400'>Không áp dụng cho đơn có thuốc kê đơn.</p>}
          </div>
...
export function MyCouponsPage() {
  ...
  const grouped = useMemo(() => ({
    ...
    condition: coupons.filter(c => c.userCouponStatus === 'needs_min_order' || c.userCouponStatus === 'not_started'),
  ...
  const renderList = (items: MyCoupon[], emptyText: string) => {
    ...
    return <div className='space-y-3'>{items.map(coupon => <CouponCard key={coupon._id || coupon.code} coupon={coupon} />)}</div>
  }
  ...
  return (
    <div className='space-y-6'>
      <div className='flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between'>
        <div>
          <h1 className='text-2xl font-bold text-[#0A2463]'>Ưu đãi của tôi</h1>
          <p className='mt-1 text-sm text-gray-500'>Theo dõi mã công khai và mã dành riêng cho tài khoản của bạn.</p>
        </div>
      ...
      <div className='grid gap-4 sm:grid-cols-3'>
        <Card className='border-[#E8EDF5]'><CardContent className='p-4'><p className='text-sm text-gray-500'>Có thể dùng</p><p className='mt-1 text-2xl font-bold text-green-700'>{grouped.usable.length}</p></CardContent></Card>
        <Card className='border-[#E8EDF5]'><CardContent className='p-4'><p className='text-sm text-gray-500'>Cần điều kiện</p><p className='mt-1 text-2xl font-bold text-yellow-700'>{grouped.condition.length}</p></CardContent></Card>
        <Card className='border-[#E8EDF5]'><CardContent className='p-4'><p className='text-sm text-gray-500'>Hết hạn/đã dùng</p><p className='mt-1 text-2xl font-bold text-gray-700'>{grouped.unavailable.length}</p></CardContent></Card>
      </div>
      ...
      {isLoading ? (
        ...
        <Tabs defaultValue='usable' className='space-y-4'>
          ...
          <TabsContent value='condition'>{renderList(grouped.condition, 'Không có mã nào đang chờ đủ điều kiện.')}</TabsContent>
          <TabsContent value='unavailable'>{renderList(grouped.unavailable, 'Chưa có mã hết hạn hoặc đã dùng hết lượt.')}</TabsContent>
        </Tabs>
      )}
    </div>
  )
